...
const parallaxContainer=document.querySelector(".parallax-container"),parallaxImage=document.querySelector(".parallax-image");function changeText(){const e=document.getElementById("myDiv");let t=0;const a=[" +_+"," *_*"," 0_0"," $_$"," @_@"," x_x"," O_o"," >_<"," ·.·"," o_o"," ^-^",];let n="",l=0;setInterval((()=>{l<a[t].length?(n+=a[t][l],e.textContent=n,l++):setTimeout((()=>{t=(t+1)%a.length,n="",l=0}),1e3)}),100)}window.addEventListener("scroll",(()=>{const e=window.scrollY,t=parallaxContainer.offsetTop,a=(parallaxContainer.offsetHeight,.5*(e-t));parallaxImage.style.transform=`translateY(${Math.min(0,a)}px)`})),changeText();
...
//     saturationMultiplier: 1e3,
//     textChangeInterval: 1e4,
...
//     particles = [];
// for (let t = 0; t < config.particleCount; t++) particles.push({ x: 0, y: 0, baseX: 0, baseY: 0, vx: 0, vy: 0 });
// const vertexShaderSource = `\n        attribute vec2 a_position;\n        attribute float a_hue;\n        attribute float a_saturation;\n        varying float v_hue;\n        varying float v_saturation;\n        void main() {\n            gl_PointSize = ${config.particleSize.toFixed(
//         1
//     )};\n            gl_Position = vec4(a_position, 0.0, 1.0);\n            v_hue = a_hue;\n            v_saturation = a_saturation;\n        }\n    `,
//     fragmentShaderSource =
//         "\n    precision mediump float;\n    varying float v_hue;\n    varying float v_saturation;\n    void main() {\n      vec3 color = vec3(1.0, 0.28, 0.08); // #FA4C14\n      gl_FragColor = vec4(color, 1.0);\n    }\n  ";
// function createShader(t, e, a) {
//     const r = t.createShader(e);
//     return t.shaderSource(r, a), t.compileShader(r), t.getShaderParameter(r, t.COMPILE_STATUS) ? r : (console.error(t.getShaderInfoLog(r)), t.deleteShader(r), null);
// }
// function createProgram(t, e, a) {
//     const r = t.createProgram();
//     return t.attachShader(r, e), t.attachShader(r, a), t.linkProgram(r), t.getProgramParameter(r, t.LINK_STATUS) ? r : (console.error(t.getProgramInfoLog(r)), t.deleteProgram(r), null);
// }
...
//     positions = new Float32Array(2 * config.particleCount),
//     hues = new Float32Array(config.particleCount),
//     saturations = new Float32Array(config.particleCount);
// function getTextCoordinates(t) {
//     const e = document.createElement("canvas").getContext("2d");
//     (e.canvas.width = canvas.width), (e.canvas.height = canvas.height);
//     const a = Math.min(canvas.width / 6, canvas.height / 6);
//     (e.font = `900 ${a}px Arial`), (e.fillStyle = "white"), (e.textAlign = "center"), (e.textBaseline = "middle"), e.fillText(t, canvas.width / 2, canvas.height / 2);
//     const r = e.getImageData(0, 0, canvas.width, canvas.height).data,
//         n = [];
//     for (let t = 0; t < canvas.height; t += 4)
//         for (let e = 0; e < canvas.width; e += 4) {
//             r[4 * (t * canvas.width + e) + 3] > 128 && n.push({ x: (e / canvas.width) * 2 - 1, y: (t / canvas.height) * -2 + 1 });
//         }
//     return n;
// }
// function createParticles() {
//     textCoordinates = getTextCoordinates(config.textArray[currentTextIndex]);
//     for (let t = 0; t < config.particleCount; t++) {
//         const e = Math.floor(Math.random() * textCoordinates.length),
//             { x: a, y: r } = textCoordinates[e];
//         (particles[t].x = particles[t].baseX = a), (particles[t].y = particles[t].baseY = r);
//     }
// }
...
//             c = mouse.radius,
//             s = (c - n) / c,
//             l = i * s * config.forceMultiplier,
//             g = o * s * config.forceMultiplier,
//             u = Math.atan2(r, a),
//             h = Math.sin(-Math.cos(-1 * u) * Math.sin(config.rotationForceMultiplier * Math.cos(s)) * Math.sin(n * n) * Math.sin(u * n)),
//             d = Math.sin(Math.cos(1 * u) * Math.sin(config.rotationForceMultiplier * Math.sin(s)) * Math.sin(n * n) * Math.cos(u * n));
//         n < mouse.radius ? ((e.vx -= l + h), (e.vy -= g + d)) : ((e.vx += (e.baseX - e.x) * config.returnSpeed), (e.vy += (e.baseY - e.y) * config.returnSpeed)),
//             (e.x += e.vx),
//             (e.y += e.vy),
//             (e.vx *= config.velocityDamping),
//             (e.vy *= config.velocityDamping);
//         Math.sqrt(e.vx * e.vx + e.vy * e.vy), config.colorMultiplier;
//         (positions[2 * t] = e.x), (positions[2 * t + 1] = e.y);
//     }
...
//     (mouse.x = ((t.clientX - e.left) / canvas.width) * 2 - 1), (mouse.y = (-(t.clientY - e.top) / canvas.height) * 2 + 1);
// }),
//     canvas.addEventListener("mouseleave", () => {
//         (mouse.x = -500), (mouse.y = -500);
//     }),
//     window.addEventListener("resize", () => {
//         (canvas.width = window.innerWidth), (canvas.height = window.innerHeight), gl.viewport(0, 0, canvas.width, canvas.height), createParticles();
//     }),
